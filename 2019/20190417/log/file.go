...
const PATH = "/Users/niuqiang/tmp/c4b39a80-a91f-4911-94f1-320114e73bd2"
...
func GenerateFileName(path string) string {
	...
	return fmt.Sprintf("%s/%s.log", path, time.Now().Format("20060102"))
}
...
func getFileNames() []string {
	...
	for _, file := range files {
		if !file.IsDir() {
			if currentLogFileName == fmt.Sprintf("%s/%s", PATH, file.Name()) {
				continue
			}
